...
def formNonlinearFunction(snes, r, f):
    """
    Evaluates a non-linear function 

    Lets go with the velocity field
    """
    ...
    print("{:03d}  residual dV = {:4e},  dP = {:4e},  dx = {:4e}".format(count, res1, res2, res3))
...
gvec.setSizes((mesh.subMesh.nodesLocal, mesh.subMesh.nodesGlobal))
gvec.setFromOptions()
gvec.setValues

print mesh.nodesLocal, mesh.nodesGlobal

# snes = PETSc.SNES().create(comm=comm)
# snes.setFunction(formNonlinearFunction, F)
# snes.setTolerances(max_it=md.maxIts)
# snes.setFromOptions()
